...
import { getInsights } from 'apiSdk/insights';
...
function DashboardPage() {
  ...
  const subscriberStat = useSWR<PaginatedInterface<SubscriberInterface>>(
    () => `/subscribers`,
    () => getSubscribers({ limit: 1, offset: 0 }),
  );

  const insightStat = useSWR<PaginatedInterface<InsightInterface>>(
    () => `/insights`,
    () => getInsights({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Subscribers',
      result: subscriberStat?.data?.totalCount,
      isLoading: subscriberStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Insights',
      result: insightStat?.data?.totalCount,
      isLoading: insightStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
