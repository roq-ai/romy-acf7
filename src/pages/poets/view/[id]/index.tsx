...
import { getPoetById } from 'apiSdk/poets';
import { PoetInterface } from 'interfaces/poet';

function PoetViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<PoetInterface>(
    () => (id ? `/poets/${id}` : null),
    () =>
      getPoetById(id, {
        relations: [],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('poet', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/poets/edit/${id}`} passHref legacyBehavior>
                    <Button
